    ...
    private static func convert(_ queryResult: OpaquePointer, row: Int32, column: Int32) -> Any {
        ...
        if PQfformat(queryResult, column) == 0 {
            ...
            case .xml:
              ...
              case .textarray:
                let numDims = Int32(bigEndian: value.withMemoryRebound(to: Int32.self, capacity: 1) { $0.pointee })
                let nullBitmap = Int32(bigEndian: (value + 4).withMemoryRebound(to: Int32.self, capacity: 1) { $0.pointee })
                let elementOID = Int32(bigEndian: (value + 8).withMemoryRebound(to: Int32.self, capacity: 1) { $0.pointee })
                let dimSize = Int32(bigEndian: (value + 12).withMemoryRebound(to: Int32.self, capacity: 1) { $0.pointee })
                let startIdx = Int32(bigEndian: (value + 16).withMemoryRebound(to: Int32.self, capacity: 1) { $0.pointee })
                print("dims: \(numDims), nullBitmap: \(nullBitmap), OID: \(elementOID), dimSize: \(dimSize)")
                var array : [Any?] = []
                var next = value.advanced(by: 20)
                for _ in 0..<dimSize {
                  let size = Int32(bigEndian: next.withMemoryRebound(to: Int32.self, capacity: 1) { $0.pointee })
                  print("size: \(size)")
                  next = next.advanced(by: 4)
                  array.append(String(cString: next))
                  next = next.advanced(by: Int(size))
                }
                print("list: \(array)")
                return array
